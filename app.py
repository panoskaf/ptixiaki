...
from tkinter.constants import  END, HORIZONTAL, INSERT
...
functions=('Πλήθος δημοσιεύσεων ανα έτος από όλα τα μέλοι του τμήματος μαζί (5ετία)', 
...
def import_jsonfile():
    ...
    if clicked.get() != "Επιλέξτε Καθηγητή":
        json_name = askopenfilename(initialdir="C:/Users/Panos/Documents",
            ...
            for paper in jdata:
                if REQUIRED_FIELDS.issubset(set(paper.keys())) : # elegxos gia kena pedia sto arxeio json
                    if paper['uid'] in list_ids : # an t ar8ro uparxei idi, enimerose t cites
                        ...
                        if paper['uid'] not in list_paper_ids: 
                        ...
                        c.execute("INSERT INTO papers(uidd, types, title, authors, years, source, cites) VALUES(:uidd, :types, :title, :authors, :years, :source, :cites)",
                                {
                                    'uidd' : paper['uid'],
                                    'types' : paper['type'],
                                    'title' : paper['title'],
...
def get_clicked(*args):
    ...
    if function_number == 0:
        ...
        pop1 = Toplevel(root)
        pop1.title("Πλήθος δημοσιεύσεων ανα έτος από όλα τα μέλοι του τμήματος μαζί (5ετία)")
        ...
        figure1 = plt.figure()
        ax = figure1.subplots()
        ax.set_title('Πλήθος δημοσιεύσεων ανα έτος από όλα τα μέλοι του τμήματος μαζί (5ετία)')
...
drop2 = OptionMenu(button_frame, clicked2 , *functions, command=get_clicked)
...
#παραθυρο διαγραφή νεου καθηγητη      
def delete_data_creation():
    pop = Toplevel(root)
    ...
    dbutton = Button(pop, text="Διαγραφή",height =" 3", width = "10",bg="#d13636",fg='white',
                     command=lambda: delete_data(mylist.curselection() , pop))
    dbutton.grid(row=0, column=1, padx=10)
  
    
#lambda διαγραφη νεου καθηγητη    
def delete_data(indices_to_delete, pop):
    global Data, list_of_entries
    ids_to_delete =[]
    names_to_delete=[]
    ...
    try:
        ...
        sqlquery1 =c.execute("""SELECT papers.uidd FROM papers
           ...
           # JOIN person_paper ON papers.uidd = person_paper.paper_id 
           # WHERE person_paper.person_id = (""" + " OR ".join(["?"] * len(ids_to_delete)) + ")",ids_to_delete).fetchall() 
        ...
        print(sqlquery2,"sql2")            
        # elegxos gia keno sqlquery output
        #if
        dup  = set(sqlquery1)           
        dup2 = set(sqlquery2)  # ta kano set gia n afero t duplicates
        
        dup_dup = list(dup-dup2)
        print(dup_dup)     
        print("sets")
        c.execute("""DELETE FROM papers 
                  WHERE  papers.uidd IN (""" + ", ".join(["?"] * len(dup_dup)) + ")",dup_dup)             
        print("dup dup")                
        c.execute("""DELETE FROM person_paper
        ...
        with open('optnames.csv','w',encoding="utf-8 -sig",newline='') as file:
           ...
           for row in enumerate(Data):
               if row[0] not in indices_to_delete: 
                   writer.writerow(row[1])  # write all non-matching rows 
...
# συναρτησεις για κουμπι αντιγραφης στ συναρτηση scolar_info    
def el_copy():
...
del_person_button = Button(button_frame, text="Διαγραφη Καθηγητή",width=20 , font=("Helvetica", 16), command=delete_data_creation)

# κουμπι πληροφοριων καθηγητη
scholar_info_button = Button(button_frame, text="Scholar Info", font=("Helvetica", 16), command=scolar_info)
...
def size_down():
    root.geometry("1255x660+30+20")

b1 = Button(root, text="1080",command= size_up)
b1.grid(row=0, column=0, sticky=tk.W ,padx=10, pady=10)
b1.place(relx=.1, rely=.03, anchor="c") 

# κουμπι εισαγωγης δεδομενων
b2 = Button(root, text="720",command= size_down)
b2.grid(row=0, column=0 ,padx=10, pady=10)
b2.place(relx=.15, rely=.03, anchor="c") 
